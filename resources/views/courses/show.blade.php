...
@section('content')
    ...
    <div class="relative bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 text-white overflow-hidden">
        ...
        <div class="absolute inset-0 opacity-10">
            ...
            <div class="absolute bottom-0 right-0 w-96 h-96 bg-purple-500 rounded-full mix-blend-overlay filter blur-3xl"></div>
        </div>
            ...
            <div class="lg:grid lg:grid-cols-3 lg:gap-8">
                ...
                <div class="lg:col-span-2">
                    ...
                    <div class="flex flex-wrap items-center gap-4 mb-6">
                        {{-- Level Badge --}}
                        <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold backdrop-blur-sm
                    ...
                    <div class="flex items-center">
                        <div class="h-12 w-12 rounded-full bg-gradient-to-br from-primary-500 to-purple-600 flex items-center justify-center text-white font-bold text-lg mr-3">
                    ...
                    <div class="relative rounded-2xl overflow-hidden shadow-2xl">
                        @if($course->thumbnail)
                            <img src="{{ Storage::url($course->thumbnail) }}" 
                                ...
                                class="w-full h-auto">
                        @else
                            <div class="w-full aspect-video bg-gradient-to-br from-primary-500 to-purple-600 flex items-center justify-center">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-2 md:grid-cols-4 gap-6 py-6">
                <div class="text-center">
                    <div class="inline-flex items-center justify-center w-12 h-12 bg-blue-100 rounded-xl mb-2">
                ...
                <div class="text-center">
                    <div class="inline-flex items-center justify-center w-12 h-12 bg-green-100 rounded-xl mb-2">
                        <svg class="w-6 h-6 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" />
                        </svg>
                    </div>
                    <p class="text-2xl font-bold text-gray-900">{{ $course->getTotalLessonsAttribute() }}</p>
                    <p class="text-sm text-gray-600">Video Materi</p>
                </div>

                <div class="text-center">
                    <div class="inline-flex items-center justify-center w-12 h-12 bg-purple-100 rounded-xl mb-2">
                        <svg class="w-6 h-6 text-purple-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                    </div>
                    <p class="text-2xl font-bold text-gray-900">∞</p>
                    <p class="text-sm text-gray-600">Akses Selamanya</p>
                </div>

                <div class="text-center">
                    <div class="inline-flex items-center justify-center w-12 h-12 bg-yellow-100 rounded-xl mb-2">
                    ...
                    <div class="bg-white rounded-t-xl border border-gray-200 border-b-0">
                        <div class="flex overflow-x-auto" x-data="{ activeTab: 'curriculum' }">
                            <button @click="activeTab = 'curriculum'" 
                                :class="activeTab === 'curriculum' ? 'border-primary-600 text-primary-600' : 'border-transparent text-gray-600 hover:text-gray-900 hover:border-gray-300'"
                                class="px-6 py-4 border-b-2 font-medium text-sm whitespace-nowrap transition">
                                📚 Kurikulum
                            </button>
                            <button @click="activeTab = 'overview'" 
                                :class="activeTab === 'overview' ? 'border-primary-600 text-primary-600' : 'border-transparent text-gray-600 hover:text-gray-900 hover:border-gray-300'"
                                class="px-6 py-4 border-b-2 font-medium text-sm whitespace-nowrap transition">
                                📖 Overview
                            </button>
                            <button @click="activeTab = 'reviews'" 
                                :class="activeTab === 'reviews' ? 'border-primary-600 text-primary-600' : 'border-transparent text-gray-600 hover:text-gray-900 hover:border-gray-300'"
                                class="px-6 py-4 border-b-2 font-medium text-sm whitespace-nowrap transition">
                                ⭐ Review ({{ $course->reviews->count() }})
                            </button>
                            <button @click="activeTab = 'discussion'" 
                                :class="activeTab === 'discussion' ? 'border-primary-600 text-primary-600' : 'border-transparent text-gray-600 hover:text-gray-900 hover:border-gray-300'"
                                class="px-6 py-4 border-b-2 font-medium text-sm whitespace-nowrap transition">
                                💬 Diskusi
                            </button>
                        </div>
                    ...
                    <div class="bg-white rounded-b-xl border border-gray-200 p-6" x-data="{ activeTab: 'curriculum' }">
                            ...
                            @if ($course->modules->count() > 0)
                                <div class="space-y-4">
                                    @foreach ($course->modules as $moduleIndex => $module)
                                        <div class="border border-gray-200 rounded-xl overflow-hidden" x-data="{ open: {{ $moduleIndex === 0 ? 'true' : 'false' }} }">
                                            {{-- Module Header --}}
                                            <button @click="open = !open" class="w-full px-6 py-4 bg-gray-50 hover:bg-gray-100 transition flex items-center justify-between">
                                                <div class="flex items-center flex-1 text-left">
                                                    <div class="flex-shrink-0 w-10 h-10 bg-primary-100 rounded-lg flex items-center justify-center mr-4">
                                                        <span class="text-primary-600 font-bold">{{ $moduleIndex + 1 }}</span>
                                                    </div>
                                                    <div>
                                                        <h3 class="text-lg font-bold text-gray-900">{{ $module->title }}</h3>
                                                        @if ($module->description)
                                                            <p class="text-sm text-gray-600 mt-0.5">{{ strip_tags($module->description) }}</p>
                                                        @endif
                                                        <p class="text-xs text-gray-500 mt-1">{{ $module->lessons->count() }} Materi</p>
                                                    </div>
                                                </div>
                                                <svg class="w-5 h-5 text-gray-400 transform transition" :class="open ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        ...
                                                        @foreach ($module->lessons as $lesson)
                                                            <li class="px-6 py-4 hover:bg-gray-50 transition flex items-center justify-between group">
                                                                <div class="flex items-center flex-1">
                                                                    {{-- Icon --}}
                                                                    @if ($lesson->video_url)
                                                                        <div class="flex-shrink-0 w-10 h-10 bg-blue-100 rounded-lg flex items-center justify-center mr-4 group-hover:bg-blue-200 transition">
                                                                    ...
                                                                    @else
                                                                        <div class="flex-shrink-0 w-10 h-10 bg-gray-100 rounded-lg flex items-center justify-center mr-4 group-hover:bg-gray-200 transition">
                                                                    ...
                                                                    {{-- Title --}}
                                                                    <span class="text-gray-900 font-medium group-hover:text-primary-600 transition">{{ $lesson->title }}</span>
                                                                </div>
                                ...
                                <div class="space-y-6">
                                    @foreach ($course->reviews->take(10) as $review)
                                        <div class="flex items-start pb-6 border-b border-gray-100 last:border-0">
                                            {{-- Avatar --}}
                                            <div class="flex-shrink-0 w-12 h-12 rounded-full bg-gradient-to-br from-primary-500 to-purple-600 flex items-center justify-center text-white font-bold text-lg">
                        ...
                        <div x-show="activeTab === 'discussion'" x-transition x-cloak>
                            ...
                            <div class="text-center py-12">
                                ...
                                <a href="{{ route('forum.index') }}" class="inline-flex items-center text-primary-600 hover:text-primary-700 font-medium">
                                    Kunjungi Forum →
                                </a>
                            </div>
                        </div>
                        ...
                        @if ($userEnrollment)
                            ...
                            <div class="mb-6">
                                <div class="flex items-center justify-between mb-3">
                                    ...
                                    <span class="text-sm font-bold text-primary-600">{{ number_format($userEnrollment->progress_percentage, 0) }}%</span>
                                </div>
                                ...
                                <p class="text-xs text-gray-500 mt-2">
                                    {{ $userEnrollment->completed_lessons_count }} dari {{ $course->getTotalLessonsAttribute() }} materi selesai
                                </p>
                            </div>
                            ...
                            @if ($userEnrollment->completed_at)
                                <div class="flex items-center justify-center px-4 py-3 bg-green-50 border-2 border-green-200 rounded-xl mb-6">
                        ...
                        <div class="border-t border-gray-200 pt-6">
                            ...
                            <ul class="space-y-3">
                                <li class="flex items-start text-sm text-gray-700">
                                    ...
                                    {{ $course->getTotalLessonsAttribute() }} video materi pembelajaran
                                </li>
                                <li class="flex items-start text-sm text-gray-700">
                                    <svg class="w-5 h-5 text-green-500 mr-2 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd" />
